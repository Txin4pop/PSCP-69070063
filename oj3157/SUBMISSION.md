...
```text
so the input , 1st line will be N how many time will loop and next line to N line is a comman that have + and - if + then the score will incress by 10 if - then the score will be minus 5
```
...
```text
Step 1:Recive input
Step 2:check if the input is + or -
...
```text
to check if there any problem with negative result
```
